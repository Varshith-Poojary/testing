...
# --- 1. INJECT PATH LOGIC (MUST RUN FIRST) ---

# A. Inject lib_py path (for modules like mesh_depth_lib_bp)
LIB_PY_ABSOLUTE_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'lib_py'))
if LIB_PY_ABSOLUTE_PATH not in sys.path:
    sys.path.insert(0, LIB_PY_ABSOLUTE_PATH)

# B. Inject Repository Root path (for package imports like 'smpl')
REPO_ROOT_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
if REPO_ROOT_PATH not in sys.path:
    sys.path.insert(0, REPO_ROOT_PATH)

# ----------------------------------------------------
# 2. STANDARD LIBRARY IMPORTS (OK to be anywhere)
# ----------------------------------------------------
import numpy as np
import torch
...
# ----------------------------------------------------
# 3. LOCAL MODULE IMPORTS (MUST RUN AFTER PATH FIX)
# ----------------------------------------------------

# These imports now rely on the path injected in Step 1
from visualization_lib_bp import VisualizationLib
from kinematics_lib_bp import KinematicsLib
from mesh_depth_lib_bp import MeshDepthLib # The former crash point!
