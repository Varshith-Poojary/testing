...
# txtfile.close()
txtfile = open("FILEPATH.txt")
REPO_ROOT = txtfile.read().strip()
txtfile.close()

LIB_PY_PATH = os.path.join(REPO_ROOT, 'lib_py')
SMPL_WEBUSER_PATH = os.path.join(REPO_ROOT, 'smpl', 'smpl_webuser3')


# --- 3. Inject these absolute paths into the system search path ---
if LIB_PY_PATH not in sys.path:
    sys.path.insert(0, LIB_PY_PATH)

if SMPL_WEBUSER_PATH not in sys.path:
    sys.path.insert(0, SMPL_WEBUSER_PATH)
...
from mesh_depth_lib_bp import FILEPATH
