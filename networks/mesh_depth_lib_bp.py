...
import random
from scipy import ndimages
import scipy.stats as ss
from scipy.ndimage.interpolation import zoom

# 1. Get the directory of the currently executing script (lib_py)
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))

# 2. Construct the absolute path to FILEPATH.txt by going up one directory
FILEPATH_LOC = os.path.join(SCRIPT_DIR, '..', 'FILEPATH.txt')

# 3. Open the file using the derived absolute path
with open(FILEPATH_LOC, 'r') as txtfile:
    # Read the absolute path of the repository from the configuration file
    FILEPATH = txtfile.read().strip() 
    
# --- The rest of the original script follows, continuing to use the variable FILEPATH ---
# txtfile.close() # Keep this if it was in the original script before the fix.
